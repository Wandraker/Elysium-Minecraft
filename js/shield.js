...
async function submitShieldToken(code, token, status, retryButton) {
  ...
  try {
    ...
    if (session?.csrfToken) {
      headers["X-Elysium-CSRF"] = session.csrfToken;
    }
...
function updateVerifyAccountHint(session) {
  ...
  if (session?.authenticated) {
    ...
    hint.className = "shield-account-hint";
    hint.querySelector("strong").textContent = "Верификация без аккаунта";
    hint.querySelector("span").textContent =
      "Вход на сервер сработает, но управлять ником в кабинете вы не сможете.";
    action.textContent = "Войти через Telegram";
    action.onclick = () => window.ElysiumAccount?.beginTelegramLogin();
  }
}
